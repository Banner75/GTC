...
namespace ZGZY.Common
{
    ...
    public class Utils
    {
        ...
        /// <param name="max">随机数上限值（不包括上限值）</param>
        /// <returns></returns>
        public static ArrayList GetRandom(int num, int max)
        {
            ...
            for (int i = 0,j=0; hashtable.Count < RmNum; i++)
            {
            ...
            return ids; //result.Trim(",".ToCharArray());
        }
        ...
        /// <returns></returns>
        public static string GetPostData(string encoding)
        {
            try
            {
                ...
                while ((count = s.Read(buffer, 0, 1024)) > 0)
                {
                    ...
                    builder.Append(encod.GetString(buffer, 0, count));
                }
                s.Flush();
                s.Close();
                s.Dispose();
        ...
        /// 将long型数据转换成日期格式
        /// </summary>
        /// <param name="ticks">DateTime.Ticks值</param>
        /// <param name="dtStr">要转换成的日期格式</param>
        /// <returns></returns>
        public static string GetDateByTick(long ticks, DateTimeType dtStr)
        {
            string date_time = "";
            ...
            switch (dtStr)
            {
                case DateTimeType.ShortDate:
                    ...
                    break;
                case DateTimeType.ShortDateTime:
                    date_time = date.ToString("yyyy-MM-dd HH:mm:ss");
                    break;
                case DateTimeType.ShortTime:
                    date_time = date.ToString("yy-MM-dd HH:mm");
                    break;
                case DateTimeType.LongDate:
                    date_time = date.ToLongDateString();
                    break;
                case DateTimeType.ShortDateNoSecond:
                    date_time = date.ToString("yyyy-MM-dd HH:mm");
                    break;
                case DateTimeType.LongDateTime:
                    date_time = date.ToString("yyyy年M月d日星期x H时m分s秒");
                    string week = "";
        ...
        /// <param name="str"></param>
        /// <returns></returns>
        public static string EncryptionParameter(string str)
        {
            return AES.Encode(str, "dfhuwerigsuzdSDRIURTBUSWGSDRKJIhuxhrsyqwe923746fshafyiaghuoegR");
        }
        /// <summary>
        /// 解密参数
        /// </summary>
        /// <param name="str"></param>
        /// <returns></returns>
        public static string DecryptionParameter(string str)
        {
            return AES.Decode(str, "dfhuwerigsuzdSDRIURTBUSWGSDRKJIhuxhrsyqwe923746fshafyiaghuoegR");
        }
        /// <summary>
        /// 生成随机数字
        /// </summary>
        ...
        public static string GetImgUrl(object HTMLStr)
        {
            if (HTMLStr == null)
                return "";
        ...
        public static string GetImgUrl(object HTMLStr, string notimg)
        {
            if (HTMLStr == null)
                return notimg;
            string str = string.Empty;
        ...
        public static void WriteLFSCookie(int stype, string openid, string unionid, string account)
        {
            HttpCookie cookie = new HttpCookie("lfs");
            cookie.Values["stype"] = stype.ToString();
            cookie.Values["openid"] = openid;
            cookie.Values["unionid"] = unionid;
            cookie.Values["account"] = Utils.UrlEncode(account);
            HttpContext.Current.Response.AppendCookie(cookie);
        }
        #endregion
