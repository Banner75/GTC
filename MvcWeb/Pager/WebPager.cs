        ...
        public sealed override string ToString()
        {
            ...
            if (_showfirst)
            {
                if (_pagemodel.IsFirstPage)
                    ...
                    html.AppendFormat("<a onclick=\"SonSubMenuItemClick('{0}')\"> 首页</a> ", CreateUrl(1));
            }

            if (_showpre)
            {
                if (_pagemodel.HasPrePage)
                    html.AppendFormat("<a onclick=\"SonSubMenuItemClick('{0}')\">上一页</a> ", CreateUrl(_pagemodel.PrePageNumber));
                else
            ...
            if (_showitems)
            {
                ...
                for (int i = startPageNumber; i <= endPageNumber; i++)
                {
                    if (_pagemodel.PageNumber == i)
                        ...
                        html.AppendFormat("<a onclick=\"SonSubMenuItemClick('{1}')\">{0}</a> ", i, CreateUrl(i));
                }
            }
            ...
            if (_shownext)
            {
                if (_pagemodel.HasNextPage)
                    html.AppendFormat("<a onclick=\"SonSubMenuItemClick('{0}')\">下一页</a> ", CreateUrl(_pagemodel.NextPageNumber));
                else
            ...
            if (_showlast)
            {
                if (_pagemodel.IsLastPage)
                    ...
                    html.AppendFormat("<a onclick=\"SonSubMenuItemClick('{0}')\">末页</a>", CreateUrl(_pagemodel.TotalPages));
            }
